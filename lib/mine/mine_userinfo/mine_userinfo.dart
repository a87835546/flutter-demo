...
class _UserInfoPageState extends State<UserInfoPage> {
                            ...
                            children: [
                              ...
                              Expanded(
                                child: Container(
                                  ...
                                  child: Text(
                                    [
                                      AppSingleton.userInfoModel?.loginName ??
                                          _nickname,
                                      AppSingleton.userInfoModel?.realName ??
                                          _realName,
                                      AppSingleton.userInfoModel?.realName ??
                                          _realName,
                                      AppSingleton.userInfoModel?.gender ??
                                              _gender
                                          ? "男"
                                          : "女"
                                    ][index],
                          ...
                          child: Row(
                            children: [
                              ...
                              Expanded(
                                ...
                                child: Text(
                                    [
                                      AppSingleton.userInfoModel?.birthday ??
                                          _birthday,
                                      AppSingleton.userInfoModel?.mobile ??
                                          _phone,
  ...
  void modifyAvatar(String url) async {
    await HttpManager.post(url: "user/modifyAvatar", params: {'url': url})
        .then((value) {
      if (value['code'] == 200) {
  ...
  void modifyNickName() async {
    await HttpManager.post(
        ...
        params: {'nickname': '12312'}).then((value) {
      if (value['code'] == 200) {
        UserInfoModel model = UserInfoModel.jsonToObject(value['data']);
        setState(() {
  ...
  void modifyRealName() async {
    await HttpManager.post(url: "user/modifyRealName", params: {'url': ''})
        .then((value) {
      if (value['code'] == 200) {
        UserInfoModel model = UserInfoModel.jsonToObject(value['data']);
        setState(() {
  ...
  void modifyGender() async {
    await HttpManager.post(
        url: "user/modifyGender",
        params: {'gender': _gender.toString()}).then((value) {
      if (value['code'] == 200) {
        UserInfoModel model = UserInfoModel.jsonToObject(value['data']);
        setState(() {
  ...
  void modifyBirthday() async {
    await HttpManager.post(
        url: "user/modifyBirthday",
        params: {'birthday': _birthday}).then((value) {
      if (value['code'] == 200) {
        UserInfoModel model = UserInfoModel.jsonToObject(value['data']);
        setState(() {
  ...
  void modifyPhone() async {
    await HttpManager.post(
        ...
        params: {'phone': '13800138000'}).then((value) {
      if (value['code'] == 200) {
        UserInfoModel model = UserInfoModel.jsonToObject(value['data']);
        setState(() {
          _phone = model.mobile ?? "";
  ...
  void modifyEmail() async {
    await HttpManager.post(url: "user/modifyEmail", params: {'url': ''})
        .then((value) {
      if (value['code'] == 200) {
        UserInfoModel model = UserInfoModel.jsonToObject(value['data']);
        setState(() {
  ...
  void modifyPwd() async {
    await HttpManager.post(url: "user/modifyPwd", params: {'password': ''})
        .then((value) {
      if (value['code'] == 200) {
        UserInfoModel model = UserInfoModel.jsonToObject(value['data']);
        getUserInfo();
  ...
  void selectImage() async {
    ImagePicker().pickImage(source: ImageSource.gallery).then((value) async {
      FormData formData =
          FormData.fromMap({'file': await MultipartFile.fromFile(value!.path)});
      HttpManager.upload(url: 'upload/images', params: formData).then((value) {
        log('upload image result $value');
        if (value['code'] == 200 && value['data']['fileUrl'] != null) {
          modifyAvatar(value['data']['fileUrl']);
        }
      });
    });
  }
